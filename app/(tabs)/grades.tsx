    ...
    <View style={{ width: '100%', position: 'relative' }} pointerEvents={active ? 'auto' : 'none'}>
      ...
      {showContentBlock && (
        <View>
          ...
          {!error && displayedClasses.length > 0 && (
            // pt-3 keeps the first row's corner star from being clipped by the scroll view.
            <Animated.View className="pt-3" style={animationsEnabled ? contentShiftStyle : undefined}>
              {gradesView === 'list' ? (
                <ReorderableList
...
export default function Screen() {
  ...
  const openCourse = (term: string, label: string, course: Course) => {
    ...
    // Opening a class marks its change badges as seen.
    useStore.getState().dismissGradeChange(`${course.course}|${course.name}`);
    if (toolMode) {
      router.push({
